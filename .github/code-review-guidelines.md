...
Check changes against [the architecture](../docs/architecture.md) and all applicable [AGENTS.md guidance](../AGENTS.md); highlight concrete violations and cite the relevant instruction. Include the testing guidance: prefer meaningful behavior coverage over test count, parameterize shared scenarios, merge overlapping coverage, and preserve distinct failure paths without obscuring readability. Review runtime and type-only dependencies, registration and sync-tracking ownership, and the documented backup and content translation-read boundaries. These checks belong to maintainers and LLM reviewers; do not assume automated boundary enforcement.
...
Do not comment on personal naming preferences, missing comments, generic requests for more test coverage, or vague "could be cleaner" feedback. Concrete violations of applicable AGENTS.md guidance and actionable findings about low-value existing or added tests are allowed.
