...
Replace per-registration wrapper choices in `entrypoints/background.ts` with one exhaustively typed message registry. Each `ExtensionMessageMap` message must declare its handler, execution lane, dirty-tracking behavior, and badge-refresh behavior.
...
- [ ] Add a command executor/registration helper that:
...
- [ ] Replace the manual `onMessage` calls in `entrypoints/background.ts` with a single typed registry and registration pass. Declare these policies explicitly:
...
- [ ] Use the same serialized sync execution path for the periodic alarm in `entrypoints/background.ts`, including badge refresh after a pull, so alarm-driven sync cannot race message-driven mutations or bypass command policy.

- [ ] Add focused tests for the registry/executor (for example, `entrypoints/__tests__/background-commands.test.ts`):
