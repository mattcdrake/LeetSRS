...
All browsers sharing a Gist must update before resuming sync; mixed-version writers are unsupported. The original transition retained ADR-0001’s background queue; [ADR-0004](0004-background-sync-lifecycle.md) supersedes that requirement with independent local saves and shared sync attempts. Retain [ADR-0002](0002-whole-dataset-gist-sync.md)’s whole-dataset timestamp policy and defer catalog identity changes. The [canonical implementation plan](https://github.com/mattcdrake/LeetSRS/issues/371) records the completed transition and deletion criteria.
