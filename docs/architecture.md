...
The bundled question catalog is a separate, rebuildable IndexedDB database, outside learning data, backup, and sync. Background startup compares the bundled hash with the stored hash and atomically replaces catalog records and the hash when they differ. One background readiness promise covers learning storage and catalog initialization. Commands, alarms, sync, and badge updates wait for both; initialization failure rejects commands and prevents background work until a successful startup. Shared catalog lookups use the frontend ID or unique slug index and return a question only for a domain listed in its sources. Direct IndexedDB access requires an extension-origin context; content-script queries will need a background bridge.
