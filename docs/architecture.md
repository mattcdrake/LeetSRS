...
The bundled question catalog is a separate, rebuildable IndexedDB database, outside learning data, backup, and sync. Background startup compares the bundled hash with the stored hash and atomically replaces catalog records and the hash when they differ. Catalog initialization runs independently of learning-document readiness. Shared catalog lookups use the frontend ID or unique slug index and return a question only for a domain listed in its sources. Direct IndexedDB access requires an extension-origin context; content-script queries will need a background bridge.
