...
- Each migration declares its historical input and output contracts. A loader validates the preceding migration's output shape; its saver accepts its own output type. Current schemas live in named domain modules. Migrations own frozen historical contracts and never import current application schemas; runtime callers never import migration contracts. Keep historical contracts and storage keys fixed, even where this duplicates current definitions.
...
- A physical layout reader knows where values are stored and how to assemble them into a raw dataset; it does not validate a migration's data schema. Each migration's loader validates those raw values against its historical input contract. A migration that changes storage locations or assembly rules must supply and test the new layout reader.
...
- Shared physical layout readers live in `infrastructure/storage/migrations/layouts/vN.ts` and export `readDataset`. `N` is the schema version that introduced the layout, not the only version it supports. Reuse a reader while the physical layout remains compatible. When a migration introduces a new layout, retain the old reader and add one named for that migration’s version. The migration reads the old layout and writes the new one; its tests verify the result through the new reader so later migrations can use it. Never rename an existing layout module to reflect which layout is current.
...
- The Gist connection is shared through browser sync; sync status and data timestamps remain local.
