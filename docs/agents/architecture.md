...
- `entrypoints/background/index.ts` registers explicit typed handlers, validates message payloads, awaits startup, and invokes named workflows. Ordinary edits also await any active arrival refresh. Local saves run independently of network and badge work; overlapping local writes are an accepted limitation.
- `services/gist-sync.ts` owns one shared active attempt, invalidation, and one follow-up for intervening saves. Download before reading current local data. Check validity before consuming network results or starting further effects; timeout, import, reset, and connection changes obsolete attempts. Already-issued writes and uploads remain subject to the accepted narrow races.
