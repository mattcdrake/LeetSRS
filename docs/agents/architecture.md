...
- Each migration declares its historical input and output contracts. A loader validates the preceding migration's output shape; its saver accepts its own output type. Shared schemas live in `domain/schemas/vN.ts`, named for the schema version that introduced them. Keep existing contracts fixed; reuse or extend them for later versions. Application models and migrations compose these definitions with their own normalization and extra-field handling. Migrations import a specific version, never current application models; historical storage keys remain independent of layout changes.
...
- Shared physical layout readers live in `infrastructure/storage/migrations/layouts/vN.ts` and export `readDataset`. `N` is the schema version that introduced the layout, not the only version it supports. Reuse a reader while the physical layout remains compatible. When a migration introduces a new layout, it reads the old layout and writes the new one. Retain the old reader; add a reader named for the version that introduced the new layout when a later migration needs it. Never rename an existing layout module to reflect which layout is current.
...
- Gist connection persistence uses one validated browser-sync record in `infrastructure/storage/gist-connection.ts`. Runtime reads and writes use the complete record; local read/modify/write workflows run through the background queue. Migration 5 retains legacy keys for older browsers and prefers an existing combined record. This is a migration handoff, not ongoing compatibility between versions or a cross-device transaction. Explicit reset removes both representations.
- Sync, backup/reset, and local-edit tracking share local status and timestamps through `infrastructure/storage/sync-metadata.ts`.
