...
- Migration 3 removes `dayStartHour` and renames `autoClearLeetcode` to `resetEditorOnEveryProblem` in both backup data and sync storage. The current field takes precedence when present; selected values remain unchanged for later current-model validation. Save the renamed value before removing the old keys. Subsequent migrations read `layouts/v3.ts`, which reuses the compatible flat-key collector.
