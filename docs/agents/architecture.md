...
- Cards are slug-keyed with numeric dates and an optional `note` string (500 characters maximum). Empty text becomes absence; whitespace is preserved. Note commands and editors use the owning card's slug. Services own note read/modify/write workflows through card persistence; background commands serialize writes and track local edits. Note queries share the card cache prefix.
...
- Migration 004 reads `layouts/v0.ts`, validates cards and attached legacy notes, and embeds notes before deleting legacy note keys. Duplicate IDs and slug/key mismatches fail; orphaned notes are discarded. An explicitly present embedded note takes precedence, including empty text. If cleanup fails after an empty note was normalized away, a retry can restore its legacy text; recovery remains best-effort. Save and cleanup must succeed before recording completion. `layouts/v4.ts` reads the embedded-note layout; version 4 backups reject a separate `notes` field before stripping extras.
- Infrastructure owns card date codecs, stored-record validation, migrations, and card/stat persistence. Persistence adapters do not mark local edits.
