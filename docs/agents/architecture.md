...
- Startup validates the completed version, then loads, transforms, saves, and cleans up each step before recording its version. The runner assumes no dataset shape. Backup parsing validates the declared version's output contract before invoking the remaining pure transformations; missing version means version 0, validated by migration 1's input contract. This includes validating the latest version when no transformations remain.
...
- Imports discard extra record and configuration fields while validating supported fields and relationships. Historical repair and conversion stay in numbered migrations: migration 1 defaults falsy card domains to `leetcode.com`; migration 3 removes `dayStartHour` and maps `autoClearLeetcode` to `resetEditorOnEveryProblem`, with a defined current value taking precedence. Version 3 forbids both retired settings. Parsing preserves valid `dataUpdatedAt`, falls back to the required `exportDate` when absent, and never reads the clock.
