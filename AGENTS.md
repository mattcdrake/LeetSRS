...
- Keep snapshot raw-card access, note traversal, and backup/reset metadata access in `infrastructure/storage/snapshot.ts`. Reuse stats and note APIs where they preserve raw fields; keep cross-entity ordering, PAT preservation, and imported timestamp restoration in services. Snapshot helpers do not mark local edits.
