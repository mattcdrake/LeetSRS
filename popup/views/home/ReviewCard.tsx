...
import { EDITOR_RESET_AUTHORIZATION_HASH } from '@/integrations/browser/editor-reset-authorization';
...
export function ReviewCard({ card, onRate, isProcessing = false }: ReviewCardProps) {
  ...
  const problemUrl = `${getLeetcodeProblemUrl(card)}${
    settings.resetEditorOnReviewQueue ? EDITOR_RESET_AUTHORIZATION_HASH : ''
  }`;
