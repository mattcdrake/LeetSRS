...
import { activeRoadmapQueryOptions, roadmapSkipsQueryOptions } from './roadmaps';
...
export function useStorageQueryEvents() {
  ...
  useEffect(() => {
    ...
    const unwatch = [
      ...
      watch(STORAGE_KEYS.roadmapSkips, [roadmapSkipsQueryOptions.queryKey]),
    ...
    void refresh([
      ...learningKeys,
      ...connectionKeys,
      activeRoadmapQueryOptions.queryKey,
      roadmapSkipsQueryOptions.queryKey,
    ]);
    return () => {
      stopped = true;
