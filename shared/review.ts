...
function getNewCardAllowance(document: LearningDocument, now: Date) {
  ...
  return { maxNewCardsPerDay, remainingNewCards: Math.max(0, maxNewCardsPerDay - newCardsCompletedToday) };
}
...
export function buildReviewCalendar(document: LearningDocument, now: Date): Record<string, ReviewCalendarDay> {
  ...
  const { maxNewCardsPerDay, remainingNewCards } = getNewCardAllowance(document, now);
  let newCardDay = today;
  let allowance = remainingNewCards;
