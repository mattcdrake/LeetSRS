...
const en = {
  ...
  cardsView: {
    ...
    sortLabel: (sort: string) => `Sort: ${sort}`,
    sorts: { due: 'Due date', id: 'Problem ID', added: 'Recently added' },
    count: (total: number) => `${total} ${total === 1 ? 'card' : 'cards'}`,
    filteredCount: (shown: number, total: number) => `${shown} of ${total}`,
    groups: { overdue: 'Overdue', today: 'Today', week: 'Next 7 days', later: 'Later', paused: 'Paused' },
    paused: 'Paused',
    hasNote: 'Has note',
    openOnLeetcode: (title: string) => `Open ${title} on LeetCode`,
    solution: 'Solution',
    deleteCard: 'Delete card',
    loadingCards: 'Loading cards...',
    emptyTitle: 'No cards yet',
    emptyDescription: 'Problems you save from Home, Roadmaps or LeetCode appear here.',
