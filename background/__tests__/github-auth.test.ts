...
it('shares concurrent refreshes and persists rotated credentials before returning', async () => {
  ...
  const pending = Promise.all(
    [getGithubAuthorization(), getGithubAuthorization(), getGithubAuthorization()].map(async (refresh) => {
  ...
  expect(JSON.parse(String(vi.mocked(fetch).mock.calls[0][1]?.body))).toEqual({ refresh_token: saved?.refreshToken });
  release.resolve();
